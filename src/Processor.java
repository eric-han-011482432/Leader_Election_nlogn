...
public class Processor extends Thread implements Observer{
		...
		case PROBE:
			...
			if(buff == this.leftInChannel) {
				...
				if((receivedId > this.processorID) && (receivedHop < Math.pow(2, receivedPhase))) {
					sendMessageToBuffer(new Message(MessageType.PROBE, receivedId, receivedPhase, receivedHop+1),this.rightOutChannel);
				}
				if((receivedId > this.processorID) && (receivedHop >= Math.pow(2, receivedPhase))) {
			...
			if(buff == this.rightInChannel) {
				...
				if((receivedId > this.processorID) && (receivedHop < Math.pow(2, receivedPhase))) {
					sendMessageToBuffer(new Message(MessageType.PROBE, receivedId, receivedPhase, receivedHop+1),this.leftOutChannel);
				}
				if((receivedId > this.processorID) && (receivedHop >= Math.pow(2, receivedPhase))) {
		...
		case REPLY:
			...
			if(buff == this.leftInChannel) {
				...
				if(receivedId != this.processorID) {
					...
					sendMessageToBuffer(new Message(MessageType.PROBE, this.processorID, receivedPhase+1, 1), this.leftOutChannel);
				}
			}
			if(buff == this.rightInChannel) {
				...
				if(receivedId != this.processorID) {
					...
					System.out.println("Winner in this phase " + receivedPhase + " is Processor" + this.processorID);
					sendMessageToBuffer(new Message(MessageType.PROBE, this.processorID, receivedPhase+1, 1), this.rightOutChannel);
				}
			}
			break;
	...
	public void run() {
		if(this.asleep) {
			...
			sendMessageToBuffer(new Message(MessageType.PROBE, this.processorID, 0, 1), this.leftOutChannel);
			System.out.println("Processor "+this.processorID+" sending probe to Processor "+this.right.getProcessorID());
			sendMessageToBuffer(new Message(MessageType.PROBE, this.processorID, 0, 1), this.rightOutChannel);	
		}
	}
}
